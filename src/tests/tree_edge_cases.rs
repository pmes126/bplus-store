...
fn insert_duplicate_key_overwrites_value() {
    ...
    let r1 = tree.insert(k(1), b"first".to_vec()).unwrap();
    let r2 = tree.insert_with_root(k(1), b"second".to_vec(), r1.new_root_id).unwrap();
...
fn insert_and_search_min_max_u64_keys() {
    ...
    let r1 = tree.insert(k(0), b"min".to_vec()).unwrap();
    let r2 = tree.insert_with_root(k(u64::MAX), b"max".to_vec(), r1.new_root_id).unwrap();
...
fn single_key_insert_search_delete() {
    ...
    let r = tree.insert(k(42), b"only".to_vec()).unwrap();
    let base = BaseVersion { committed_ptr: tree.get_metadata() };
    tree.try_commit(&base, StagedMetadata {
